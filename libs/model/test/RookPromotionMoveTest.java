...
public class RookPromotionMoveTest {
	...
	@Test
	void shouldPromotePawnToRook() {
		new RookPromotionMove(f7, f8, board);
		board.getFen().switchActiveColor();
		board.getFen().updatePiecePlacementData(board);
		String expected = "rnbq1R1r/pppkb1pp/4pn2/8/8/1PNp1Q2/P1PP1PPP/R1B1KBNR b KQ - 0 8";
		assertEquals(expected, board.getFen().toString());
	}

	@Test
	void shouldPrint() {
		var move = new RookPromotionMove(f7, f8, board);

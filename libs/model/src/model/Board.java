...
class Board implements IBoard, Serializable {
	...
	static final byte NORTH = SIZE;
	static final byte EAST = 1;
	static final byte SOUTH = -SIZE;
	static final byte WEST = -1;
	static final byte NORTH_EAST = NORTH + EAST;
	static final byte SOUTH_EAST = SOUTH + EAST;
	static final byte SOUTH_WEST = SOUTH + WEST;
	static final byte NORTH_WEST = NORTH + WEST;
	static final byte NORTH_NORTH_EAST = NORTH + NORTH_EAST;
	static final byte NORTH_NORTH_WEST = NORTH + NORTH_WEST;
	static final byte SOUTH_SOUTH_EAST = SOUTH + SOUTH_EAST;
	static final byte SOUTH_SOUTH_WEST = SOUTH + SOUTH_WEST;
	static final byte EAST_EAST_NORTH = EAST + NORTH_EAST;
	static final byte EAST_EAST_SOUTH = EAST + SOUTH_EAST;
	static final byte WEST_WEST_NORTH = WEST + NORTH_WEST;
	static final byte WEST_WEST_SOUTH = WEST + SOUTH_WEST;
	static final IBitboard firstFile = new Bitboard(0x0101010101010101L);
	static final IBitboard secondFile = new Bitboard(0x0202020202020202L);
	static final IBitboard lastFile = new Bitboard(0x8080808080808080L);
	static final IBitboard secondLastFile = new Bitboard(0x4040404040404040L);
	static final IBitboard firstRank = new Bitboard(0x00000000000000FFL);
	static final IBitboard fourthRank = new Bitboard(0x0000000FF000000L);
	static final IBitboard fifthRank = new Bitboard(0x000000FF00000000L);
	static final IBitboard eighthRank = new Bitboard(0xFF00000000000000L);
	static final IBitboard notFirstFile = Bitboard.negate(firstFile);
	static final IBitboard notLastFile = Bitboard.negate(lastFile);

	static int getSquareIndex(int rank, int file) {
	...
	private IKing whiteKing;
	private IQueen whiteQueen;
	private IRook whiteRook;
	private IKnight whiteKnight;
	private IBishop whiteBishop;
	private IPawn whitePawn;
	private IKing blackKing;
	private IQueen blackQueen;
	private IRook blackRook;
	private IKnight blackKnight;
	private IBishop blackBishop;
	private IPawn blackPawn;
	...
	Board(ForsythEdwardsNotation fen) {
		initializeBoard();
		setFen(fen);
		setPieces();
	}

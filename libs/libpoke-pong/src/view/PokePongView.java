	...
	public void keyPressed(KeyEvent event) {
		...
		switch (state) {
			...
			case PLAYING -> {
				...
				if (event.getKey() == 'w') {
					controller.movePlayerOne(
						max((p1.getSize() / 2.0f), p1.position.y - 10)
					);
				}

				if (event.getKey() == 's') {
					controller.movePlayerOne(
						min((height - p1.getSize() / 2.0f), p1.position.y + 10)
					);
				}
				...
				if (event.getKeyCode() == UP) {
					controller.movePlayerTwo(
						max((p2.getSize() / 2.0f), p2.position.y - 10)
					);
				}
				if (event.getKeyCode() == DOWN) {
					controller.movePlayerTwo(
						min((height - p2.getSize() / 2.0f), p2.position.y + 10)
					);
				}
			}
			default -> throw new IllegalStateException(
